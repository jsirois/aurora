...
abstract class BaseVisitor<T extends Visitable> extends BaseEmitter implements Visitor<T> {
  ...
  protected static Iterable<AnnotationSpec> createAnnotations(
      List<TypeAnnotation> typeAnnotations) {

    return typeAnnotations.stream()
        .map(a -> AnnotationSpec.builder(ThriftAnnotation.class)
            .addMember("name", "$S", a.getName())
            .addMember("value", "$S", a.getValue())
            .build())
