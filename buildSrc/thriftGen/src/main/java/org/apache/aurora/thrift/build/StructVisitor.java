...
class StructVisitor extends BaseVisitor<Struct> {
  ...
  private void writeStruct(
    ...
    // We cache the hashcode since this is both safe since the generated struct is immutable and
    // provides >50% speedup in some of the jmh benchmarks (We do a fair amount on collection
    // manipulation in certain queries).
    typeBuilder.addField(int.class, "cachedHashCode", Modifier.PRIVATE, Modifier.VOLATILE);
    typeBuilder.addMethod(
        MethodSpec.methodBuilder("hashCode")
            .addAnnotation(Override.class)
            .addModifiers(Modifier.PUBLIC)
            .returns(int.class)
            .addCode("// Only access the volatile once on the fast path.\n")
            .addStatement("int result = this.cachedHashCode")
            .beginControlFlow("if (result == 0)")
            .addStatement(
                "result = $T.hash(\n$L)",
                Objects.class,
                Joiner.on(",\n").join(
                    FluentIterable.from(struct.getFields())
                        .transform(BaseEmitter::getterName)
                        .transform(g -> g + "()")))
            .addStatement("this.cachedHashCode = result")
            .endControlFlow()
