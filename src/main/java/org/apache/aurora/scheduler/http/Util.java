...
final class Util {
   ...
   */
  static <F extends ThriftFields> String prettyPrint(ThriftEntity<F> t) {
    return t == null ? "null" : printEntity(t, 0);
  }
  ...
  /**
   * Prints a ThriftEntity.
   *
   * @param entity The object to print.
   * @param depth The print nesting level.
   * @return The pretty-printed version of the TBase.
   */
  private static <F extends ThriftFields> String printEntity(ThriftEntity<F> entity, int depth) {
    List<String> fields = Lists.newArrayList();
    for (F field : entity.getFields()) {
      boolean fieldSet = entity.isSet(field);
      String strValue;
      if (fieldSet) {
