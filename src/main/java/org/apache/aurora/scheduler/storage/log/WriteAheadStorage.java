...
class WriteAheadStorage extends ForwardingStore implements
  ...
  public ImmutableSet<ScheduledTask> mutateTasks(
    ...
    ImmutableSet<ScheduledTask> mutated = taskStore.mutateTasks(query, mutator);

    Map<String, ScheduledTask> tasksById = Tasks.mapById(mutated);
    if (log.isLoggable(Level.FINE)) {
      log.fine("Storing updated tasks to log: "
          + Maps.transformValues(tasksById, ScheduledTask::getStatus));
    }

    // TODO(William Farner): Avoid writing an op when mutated is empty.
    write(Op.saveTasks(SaveTasks.create(mutated)));
    return mutated;
  }
  ...
  public void saveJobInstanceUpdateEvent(JobUpdateKey key, JobInstanceUpdateEvent event) {
    ...
    write(Op.saveJobInstanceUpdateEvent(
        SaveJobInstanceUpdateEvent.create(event, key)));
