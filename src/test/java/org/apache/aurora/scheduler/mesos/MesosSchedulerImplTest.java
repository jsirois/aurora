...
public class MesosSchedulerImplTest extends EasyMockTest {
    ...
    new OfferFixture() {
      ...
      void respondToOffer() {
        HostAttributes draining =
            OFFER.getAttributes().toBuilder().setMode(DRAINING).build();
        expect(storageUtil.attributeStore.getHostAttributes(OFFER.getOffer().getHostname()))
            .andReturn(Optional.of(draining));
        HostAttributes saved =
            Conversions.getAttributes(OFFER.getOffer()).toBuilder().setMode(DRAINING).build();
  ...
  private void expectOfferAttributesSaved(HostOffer offer) {
    ...
    HostAttributes defaultMode = Conversions.getAttributes(offer.getOffer()).toBuilder()
